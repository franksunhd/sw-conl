                        ...
                        switch (res.status) {
                            case 200:
                                ...
                                if (resData.length !== 0) {
                                    ...
                                    // 语言
                                    _t.formItem.language = resData[0].language !== undefined ? resData[0].language : 'zh_CN';
                                    // 处理缩略图
                                    if (resData[0].thumbnail !== undefined && resData[0].thumbnail !== null) {
                                        // 分为横板缩略图和竖版缩略图 默认长度为1 竖版
                                        let thumbnail = resData[0].thumbnail.split(',');
                                        if (thumbnail.length === 1) {
                                            _t.formItem.imgUrl = thumbnail[0];
                                            _t.formItem.imgStatus = false;
                                            ...
                                            _t.formItem.imgStatus = false;
                                            // 横板
                                            _t.formItem.thumbnail = thumbnail[1];
                                            _t.formItem.thumbnailStatus = false;
                                        }
                                    }
                                    // 活动标题
                                    _t.formItem.title = resData[0].activityTitle !== undefined ? resData[0].activityTitle : '';
                                    // 活动时间
                                    _t.formItem.time = resData[0].activityTime !== undefined ? new Date(Number(resData[0].activityTime)) : null;
                                    // 抢票开始时间
                                    if (resData[0].startTime !== undefined && resData[0].startTime !== null) {
                                        _t.formItem.startTime = (resData[0].startTime === 0 || resData[0].startTime === '0') ? null : new Date(Number(resData[0].startTime));
                                    }
                                    // 抢票结束时间
                                    if (resData[0].endTime !== undefined && resData[0].endTime !== null) {
                                        _t.formItem.endTime = (resData[0].endTime === 0 || resData[0].endTime === '0') ? null : new Date(Number(resData[0].endTime));
                                    }
                                    // 活动描述
                                    // _t.formItem.purpose = resData[0].activityPurpose !== undefined ? resData[0].activityPurpose : '';
                                    // 活动地点
                                    _t.formItem.address = resData[0].activityAddress !== undefined ? resData[0].activityAddress : '';
                                    // 活动场次
                                    // _t.formItem.session = resData[0].activitySession !== undefined ? new Date(Number(resData[0].activitySession)) : null;
                                    // 限购数量
                                    _t.formItem.limitNum = resData[0].activityRestriction !== undefined ? resData[0].activityRestriction.toString() : '';
                                    // 票面价格及库存
                                    let ticketNum = new Array();
                                    if (res.inventory !== undefined && res.inventory !== null) {
                                        res.inventory.forEach((item) => {
                                            var Obj = new Object();
                                            Obj.inventoryId = item.id;
                                            Obj.activityId = item.activityId;
                                            Obj.price = (item.ticketPrice / 100).toString();
                                            Obj.count = item.ticketInventory.toString();
                                            ticketNum.push(Obj);
                                        });
                                    }
                                    if (ticketNum.length !== 0) {
                                        _t.ticketNum = ticketNum;
                                    }
                                    // 处理支付问题
                                    if (res.question !== undefined && res.question !== null) {
                                        let questionList = new Array();
                                        for (var i = 0; i < 2; i++) {
                                            let questionObj = res.question[i];
                                            let Obj = new Object();
                                            if (questionObj !== undefined && questionObj !== null) {
                                                Obj.questionId = res.question[i].id;
                                                Obj.activityId = res.question[i].activityId;
                                                Obj.question = res.question[i].question !== undefined ? res.question[i].question : '';
                                                Obj.answer = res.question[i].answer !== undefined ? res.question[i].answer : '';
                                            } else {
                                                Obj.questionId = null;
                                                Obj.activityId = null;
                                                Obj.question = '';
                                        ...
                                        _t.problemList = questionList;
                                    }
                                    // 演出介绍
                                    if (resData[0].showIntroduceStr !== undefined && resData[0].showIntroduceStr !== null) {
                                        _t.formItem.editor.txt.html(resData[0].showIntroduceStr);
                                    }
                                    // 购票须知
                                    if (resData[0].buyerReadingStr !== undefined && resData[0].buyerReadingStr !== null) {
                                        var buyerReading = JSON.parse(resData[0].buyerReadingStr);
                                        _t.ticketItem.a = buyerReading.a !== undefined ? buyerReading.a : '';
                                        _t.ticketItem.b = buyerReading.b !== undefined ? buyerReading.b : '';
                                        _t.ticketItem.c = buyerReading.c !== undefined ? buyerReading.c : '';
                                        _t.ticketItem.d = buyerReading.d !== undefined ? buyerReading.d : '';
                                        _t.ticketItem.e = buyerReading.e !== undefined ? buyerReading.e : '';
                                        _t.ticketItem.f = buyerReading.f !== undefined ? buyerReading.f : '';
                                        _t.ticketItem.g = buyerReading.g !== undefined ? buyerReading.g : '';
                                    }

                                    // 演出须知
                                    if (resData[0].viewingReadingStr !== undefined && resData[0].viewingReadingStr !== null) {
                                        var viewingReading = JSON.parse(resData[0].viewingReadingStr);
                                        _t.viewItem.a = viewingReading.a !== undefined ? viewingReading.a : '';
                                        _t.viewItem.b = viewingReading.b !== undefined ? viewingReading.b : '';
                                        _t.viewItem.c = viewingReading.c !== undefined ? viewingReading.c : '';
                                        _t.viewItem.d = viewingReading.d !== undefined ? viewingReading.d : '';
                                        _t.viewItem.e = viewingReading.e !== undefined ? viewingReading.e : '';
                                        _t.viewItem.f = viewingReading.f !== undefined ? viewingReading.f : '';
                                    }
                                }
                                break;
                            default:
