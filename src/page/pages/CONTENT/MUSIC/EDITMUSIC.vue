            ...
            <div v-if="formItem.protect === false">
                ...
                <el-form-item>
                    <el-upload
                        ...
                        <i v-else-if="formItem.songUrl === '' && formItem.songUrlStatus === false" class="el-icon-upload"></i>
                        <el-progress
                            class="activity_progress"
                ...
                <el-form-item>
                    <el-upload
                        ...
                        <i v-else-if="formItem.lyrics === '' && formItem.lyricStatus === false" class="el-icon-upload"></i>
                        <el-progress
                            class="activity_progress"
                        ...
                        switch (res.status) {
                            case 200:
                                ...
                                if (resData.length !== 0) {
                                    ...
                                    _t.formItem.songName = resData[0].musicName !== undefined ? resData[0].musicName : ''; // 歌曲名
                                    _t.formItem.time = new Date(Number(resData[0].publishTime)); // 发行时间
                                    _t.formItem.group = resData[0].grouping !== undefined ? resData[0].grouping.toString() : ''; // 分组
                                    _t.formItem.sync = resData[0].showHome !== undefined ? (resData[0].showHome === 1 ? true : false) : ''; // 同步首页
                                    // 是否版权保护
                                    if (resData[0].copyrightProtection !== undefined && resData[0].copyrightProtection !== null) {
                                        // _t.formItem.protect = resData[0].copyrightProtection === 0 ? true : false; // 是否受版权保护, 0: 受 1:公开
                                    }
                                    ...
                                    // 音频文件
                                    if (resData[0].musicPath !== undefined && resData[0].musicPath !== null) {
                                        _t.formItem.songUrl = resData[0].musicPath;
                                        _t.formItem.songUrlStatus = false;
                                    }
                                    // 歌词
                                    if (resData[0].lyrics !== undefined && resData[0].lyrics !== null) {
                                        _t.formItem.lyrics = resData[0].lyrics;
                                    }
                                    ...
                                    if (resData[0].singer !== undefined && resData[0].singer !== null) {
                                        ...
                                        if (singer.length !== 0 && singer.length === 1) {
                                            _t.formItem.singerName = singer[0];
                                        } else if (singer.length !== 0 && singer.length === 2) {
                                            _t.formItem.singerName = singer[0];
                                            _t.formItem.singerEnName = singer[1];
                                        }
                                    }

                                    // 处理平台类型及地址
                                    if (resData[0].musicLikeStr !== undefined && resData[0].musicLikeStr !== null) {
                                        var musicLink = JSON.parse(resData[0].musicLikeStr);
                                        if (typeof musicLink === 'object' && musicLink.length !== undefined && musicLink.length !== 0) {
                                            var textArr = new Array();
                                            musicLink.forEach((item) => {
                                                var Obj_text = new Object();
                                                Obj_text.name = item.k;
                                                Obj_text.link = item.v;
                                                textArr.push(Obj_text);
                                            });
                                            _t.linkArr = textArr;
                                        }
                                    }
