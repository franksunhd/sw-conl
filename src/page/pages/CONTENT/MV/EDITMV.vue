<template>
    <Box>
        <el-form :model="formItem">
            ...
            <p class="star formTitle">歌手名(中文)</p>
            <el-form-item>
                <el-input
                    :class="isWeb ? 'widthFixed' : 'widthAll'"
                    @blur="blurInput(formItem, 'userNameFlag', formItem.user)"
                    v-model="formItem.user"
                    maxlength="50"
                    placeholder="请输入中文歌手名"/>
                <div v-if="formItem.userNameFlag" class="isNotNullText">必填项不能为空</div>
            </el-form-item>
            <p class="star formTitle">歌手名(英文)</p>
            <el-form-item>
                <el-input
                    :class="isWeb ? 'widthFixed' : 'widthAll'"
                    @blur="blurInput(formItem, 'userNameEnFlag', formItem.userEn)"
                    v-model="formItem.userEn"
                    maxlength="100"
                    placeholder="请输入英文歌手名"/>
                <div v-if="formItem.userNameEnFlag" class="isNotNullText">必填项不能为空</div>
            </el-form-item>
            ...
            <p class="star formTitle">发布模块</p>
            <el-form-item>
                <el-select v-model="formItem.sync">
                    <el-option label="MUSIC模块MV" :value="0"/>
                    <el-option label="首页MV" :value="1"/>
                </el-select>
            </el-form-item>
    ...
    export default {
        ...
        data() {
            return {
                ...
                formItem: {
                    ...
                    userNameFlag: false, // 歌手名标识
                    timeLengthFlag: false, // 时长标识
                    timeFlag: false, // 时间标识
                    groupFlag: false, // 分组标识
                    userNameEnFlag: false, // 英文标识
                    userEn: '', // 英文
                },
                minutesList: [], // 分钟
                secondsList: [], // 秒
        ...
        methods: {
                        ...
                        switch (res.status) {
                            case 200:
                                ...
                                if (resData.length !== 0) {
                                    ...
                                    _t.formItem.title = resData[0].videoName !== null ? resData[0].videoName : ''; // 标题
                                    // 时间
                                    _t.formItem.time = new Date(Number(resData[0].releaseTime));
                                    _t.formItem.group = resData[0].grouping.toString(); // 分组
                                    _t.formItem.sync = resData[0].showHome !== null ? resData[0].showHome : 0;
                                    // 计算时长
                                    if (resData[0].videoTime !== null) {
                                        var minutes = resData[0].videoTime / 60;
                                        var seconds = resData[0].videoTime % 60;
                                        _t.formItem.minutes = parseInt(minutes) >= 10 ? parseInt(minutes).toString() : '0' + parseInt(minutes);
                                        _t.formItem.seconds = parseInt(seconds) >= 10 ? parseInt(seconds).toString() : '0' + parseInt(seconds);
                                    }
                                    // 缩略图
                                    ...
                                    // 视频资源
                                    if (resData[0].videoPath !== undefined && resData[0].videoPath !== null) {
                                        _t.formItem.mvUrl = resData[0].videoPath;
                                        _t.formItem.mvUrlStatus = false;
                                    }
                                    // 歌手
                                    if (resData[0].singer !== undefined && resData[0].singer !== null) {
                                        var singer = JSON.parse(resData[0].singer);
                                        if (singer.length !== 0 && singer.length === 1) {
                                            _t.formItem.user = singer[0];
                                        } else if (singer.length !== 0 && singer.length === 2) {
                                            _t.formItem.user = singer[0];
                                            _t.formItem.userEn = singer[1];
                                        }
                                    }
                                }
                                break;
            ...
            addData() {
                ...
                // 校验歌手名
                _t.blurInput(_t.formItem, 'userNameFlag', _t.formItem.user);
                _t.blurInput(_t.formItem, 'userNameEnFlag', _t.formItem.userEn);
                // 校验时间
                ...
                if (_t.formItem.imgUrlFlag === false
                    ...
                    && _t.formItem.userNameFlag === false
                    && _t.formItem.userNameEnFlag === false
                    ...
                    singer[0] = _t.formItem.user.trim();
                    singer[1] = _t.formItem.userEn.trim();
                    _t.$api.post('video/saveMusicVideo/push', {
                        id: _t.addEdit.id,
                        contenId: _t.addEdit.contentId,
                        thumbnail: _t.formItem.imgUrl, // 缩略图
                        videoName: _t.formItem.title.trim(), // 标题
                        videoPath: _t.formItem.mvUrl, // 视频路径
                        singer: JSON.stringify(singer), // 歌手
                        videoTime: videoLong, // 时长
                        releaseTime: new Date(_t.formItem.time).getTime(), // 时间
                        grouping: _t.formItem.group, // 分组
                        showHome: _t.formItem.sync, // 发布模块
